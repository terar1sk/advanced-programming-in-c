...
struct bmp_image* scale(const struct bmp_image* image, float factor){
  ...
  int new_height = new_image->header->height;
...
struct bmp_image* crop(const struct bmp_image* image, const int start_y, const int start_x, const int height, const int width){
  if(image == NULL || image->header == NULL || image->data == NULL ||
  start_y < 0 || start_x < 0 || height <= 0 || width <= 0 ||
  start_y + height > image->header->height || start_x + width > image->header->width){
    return NULL;
  }
  struct bmp_image* new_image = malloc(sizeof(struct bmp_image));
    if(new_image == NULL){
    ...
    int original_height = image->header->height;
    for(int a = 0; a < height; a++){
      for(int b = 0; b < width; b++){
